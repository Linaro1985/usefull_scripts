...
UBUNTU_PATH="/home/${USER}/${UBUNTU_ROOT}"
...
function update_apt_list() {
	...
	local TEMP="deb ${UBUNTU_MIRROR} ${UBUNTU_EDITION}"

	echo "${TEMP} main restricted" > $APT_LIST_PATH
	echo "${TEMP}-updates main restricted" >> $APT_LIST_PATH
	echo "${TEMP} universe" >> $APT_LIST_PATH
	echo "${TEMP}-updates universe" >> $APT_LIST_PATH
	echo "${TEMP} xenial multiverse" >> $APT_LIST_PATH
	echo "${TEMP}-updates multiverse" >> $APT_LIST_PATH
	echo "${TEMP}-backports main restricted universe multiverse" >> $APT_LIST_PATH
	echo "${TEMP}-security main restricted" >> $APT_LIST_PATH
	echo "${TEMP}-security universe" >> $APT_LIST_PATH
	echo "${TEMP}-security multiverse" >> $APT_LIST_PATH
